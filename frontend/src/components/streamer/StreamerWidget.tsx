...
function Stat({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, minWidth: 54 }}>
      <div style={{ fontSize: 29, fontWeight: 900, color: '#fff', lineHeight: 1, fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.01em' }}>{value}</div>
      <div style={{ fontSize: 9.5, fontWeight: 800, color, textTransform: 'uppercase', letterSpacing: '0.12em' }}>{label}</div>
    </div>
  )
}

function VDiv() {
  return <div style={{ width: 1, alignSelf: 'stretch', margin: '4px 0', background: 'linear-gradient(180deg, transparent, rgba(255,255,255,0.13), transparent)' }} />
}

/**
 * OBS / стрим виджет CONDR FACEIT. Прозрачный внешний фон.
 * Бренд-локап + круглый значок ранга (EloRing) + ник + ELO/калибровка ·
 * AVG · K/D · Rating · мир/регион · форма.
 */
export function StreamerWidget({ stats }: { stats: ObsStats }) {
  ...
  return (
    <div style={{
      ...
      padding: '14px 18px 15px', fontFamily: 'inherit',
    }}>
      ...
      {/* faint CONDR logo watermark (brand) */}
      <img src="/logo_vectorized.svg" aria-hidden alt="" style={{ position: 'absolute', right: -22, top: '52%', transform: 'translateY(-50%)', width: 188, height: 188, opacity: 0.05, pointerEvents: 'none' }} />
      {/* dotted texture */}
      ...
      {/* ── Brand lockup ── */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 7, position: 'relative', marginBottom: 8 }}>
        <img src="/logo_vectorized.svg" alt="" style={{ width: 15, height: 15, opacity: 0.95 }} />
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 13, position: 'relative' }}>
        ...
        <EloRing elo={stats.elo} size={58} showLabel={false} />

        {/* Nick + ELO / calibration */}
        <div style={{ minWidth: 0, width: 162 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontSize: 19, fontWeight: 900, color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', letterSpacing: '-0.02em' }}>{stats.nickname}</span>
            {stats.isVerified && <Icon name="verified" size={15} color="#3B82F6" />}
          </div>
          {stats.calibrating ? (
            <div style={{ marginTop: 5 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 7 }}>
                <span style={{ fontSize: 21, fontWeight: 900, color: accent, fontVariantNumeric: 'tabular-nums', lineHeight: 1 }}>{stats.calibrationPlayed}<span style={{ color: '#6B7280', fontSize: 14 }}>/{stats.calibrationTotal}</span></span>
                <span style={{ fontSize: 9, fontWeight: 800, color: accent, textTransform: 'uppercase', letterSpacing: '0.1em' }}>калибровка</span>
              </div>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginTop: 4 }}>
              ...
              <span style={{ fontSize: 25, fontWeight: 900, color: '#fff', lineHeight: 1, fontVariantNumeric: 'tabular-nums', textShadow: `0 2px 14px ${accent}66` }}>{stats.elo.toLocaleString()}</span>
        ...
        {/* Stats */}
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 15 }}>
      ...
      {/* ── Hairline ── */}
      <div style={{ height: 1, margin: '12px 0 10px', background: `linear-gradient(90deg, transparent, ${accent}99, ${accent}55, transparent)` }} />
